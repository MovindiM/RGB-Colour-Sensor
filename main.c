...
#define Res 10 //Resistor connected to LDR and GND (10k)
...
void sensor(float rgbArr[3]); //Function of sensing Mode
...
float ldrValue(); //Function o read value of LDR
float getMax(float num_1,float num_2,float num_3); //Function to get maximum value
int mapValue(float value, float maximum, float minimum); //Function to map LDR value to integer number
...
void sensorOutput(float whiteArr[3], float blackArr[3], float rgbArr[3]); //Function of output (sensor Mode)
...
float ldrValue()
{
	...
	Vin = (ADC*Vref)/1024; //ADC = (Vin*1024)/Vref
	LDR = (10*Vin)/(Vref-Vin);
	return LDR;
}
...
//get maximum
float getMax(float num_1,float num_2,float num_3)
{
	float Maximum = 0;
	float Arr[3] = {num_1,num_2,num_3};
	for (int i=0;i<3;i++)
	{
		if (Arr[i]>Maximum)
		{
...
void calibrate(float whiteArr[3],float blackArr[3])
{
	float Arr_1[3] = {0,0,0};
	float Arr_2[3] = {0,0,0};
	float Arr_3[3] = {0,0,0};
...
int mapValue(float value, float maximum, float minimum)
{
	float x;
	int y;
	x = ((maximum - value)/(maximum - minimum))*255;
	y = round(x);
	return y;
}
...
void sensorOutput(float whiteArr[3], float blackArr[3], float rgbArr[3] )
{
	int value;
	char str[20];
	lcdCmd(0x01); //clear LCD display
